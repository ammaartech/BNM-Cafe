...
import { UserPaymentDialog } from "@/components/checkout/UserPaymentDialog";
...
export default function CheckoutPage() {
    ...
    const { state, totalPrice, totalItems, placeOrder, placePendingOrder, clearBackendCart } = useCart();
    ...
    const handlePayByCash = async () => {
        ...
        await placeOrder(); // Creates order, clears cart, and redirects to ticket
    }

    const handlePayByUPI = async () => {
        setIsPlacingOrder(true);
        const orderData = await placePendingOrder();
        setIsPlacingOrder(false); // Just opened dialog, not loading anymore
    ...
    const handleUpiPaymentConfirmed = async (orderId: string) => {
        ...
        <div className="flex flex-col h-full p-4">
            ...
            <div className="mt-auto pt-4 space-y-4">
                ...
                <div className="flex flex-col gap-3 pb-8">
                    ...
                    >
                        {isPlacingOrder ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <QrCode className="mr-2 h-5 w-5" />}
                        {isPlacingOrder ? 'Processing...' : 'Pay via UPI'}
                    </Button>
