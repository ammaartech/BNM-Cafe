...
import { LogOut, Search, Plus, X, Minus, Heart, User } from "lucide-react";
...
function MenuItemGridCard({ item, priority }: { item: MenuItem; priority: boolean }) {
          ...
          <div className="relative aspect-square w-full">
            {itemImage && (
              <Image
                ...
                // The first row is on screen at load; everything below lazy-loads.
                priority={priority}
...
/**
 * Mirrors `?filter=` into state. Kept in its own Suspense boundary: reading
 * search params anywhere else would opt the whole menu out of server
 * rendering, and the grid would wait for JavaScript again.
 */
function FilterFromUrl({ onChange }: { onChange: (filter: string) => void }) {
...
export default function MenuClient({ initialItems }: { initialItems: MenuItem[] | null }) {
  const { user, userProfile, supabase } = useSupabase();
  ...
  const { data: menuItemsData } = useMenuItems(initialItems);
  const menuItems = menuItemsData || [];
  // Skeleton only when there is truly nothing to show. SWR's isLoading can read
  // true on a re-render even while the server-rendered menu is on screen.
  const isLoading = !menuItemsData;
  ...
  return (
    <div className="flex flex-col h-full">
      <Suspense fallback={null}>
        <FilterFromUrl onChange={setActiveFilter} />
      </Suspense>
      ...
      <main className="flex-grow px-2 py-4 overflow-y-auto">
        {isLoading ? (
          ...
          // The grid arrives in the server HTML; starting it at opacity 0 would
          // hide it until JavaScript loads. Only filter changes animate.
          <div className="grid grid-cols-2 gap-4">
            <AnimatePresence mode="popLayout" initial={false}>
              {displayedItems.map((item, index) => (
                <motion.div
                  ...
                  key={item.id}
                >
                  <MenuItemGridCard item={item} priority={index < 4} />
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
